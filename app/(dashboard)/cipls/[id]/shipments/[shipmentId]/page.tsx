...
export default function ShipmentDetailPage() {
  ...
  useEffect(() => { loadingStartedAt.current = Date.now(); setLoading(true); getShipment(params.shipmentId).then(async (loaded) => { setShipment(loaded); if (loaded) setJobs(await listCustomsJobs(loaded.id)); finishLoadingAfterMinimum(loadingStartedAt.current, () => setLoading(false)) }) }, [params.shipmentId])
  if (loading || !shipment) return <PageSkeleton cards={2} rows={4} />
