...
export default async function Dashboard() {
  ...
  const stats = [
    { label: 'Project aktif', value: projects.filter((item) => item.status === 'ACTIVE').length, note: 'Portofolio berjalan', icon: PackageOpen, tone: 'text-indigo-700 bg-indigo-50' },
    { label: 'Task terbuka', value: openTasks, note: `${readyTasks} siap dijalankan`, icon: ClipboardList, tone: 'text-sky-700 bg-sky-50' },
    ...
    <div className="space-y-8">
      ...
      <section className="card overflow-hidden">
        <div className="flex items-center justify-between border-b px-5 py-4">
          ...
          <Link href="/projects" className="inline-flex items-center gap-1 text-sm font-semibold text-indigo-700">Lihat semua <ArrowUpRight size={15} /></Link>
        </div>
          ...
          {projects.map((project) => {
            ...
            return (
              <Link key={project.id} href={`/projects/${project.id}`} className="group grid gap-4 p-5 hover:bg-slate-50 md:grid-cols-[1fr_160px_150px] md:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-2"><p className="font-semibold group-hover:text-indigo-700">{project.name}</p><StatusBadge status={project.status} /></div>
                ...
                <div><div className="flex justify-between text-xs"><span>Progress</span><span className="font-semibold">{progress}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-indigo-600" style={{ width: `${progress}%` }} /></div></div>
